(function ($) {
    'use strict';

    const responses = {
        'We have sent you a confirmation email'                                             : 0,
        'Please enter a valid email'                                                        : 1,
        'An email address must contain a single @'                                          : 2,
        'The domain portion of the email address is invalid (the portion after the @: )'    : 3,
        'The username portion of the email address is invalid (the portion before the @: )' : 4,
        'This email address looks fake or invalid. Please enter a real email address'       : 5
    };

    const translations = {
        'en': null
    };

    const init = function (selector, options) {
        $(selector).ajaxChimp(options);
    };

    const ajaxChimp = function (options) {
        return this.each(function () {
            const form = $(this);
            const email = form.find('input[type=email]');
            const label = form.find('.info');

            const settings = $.extend({
                'url': form.attr('action'),
                'language': 'en'
            }, options);

            const url = `${settings.url.replace('/post?', '/post-json?')}&c=?`;

            form.attr('novalidate', 'true');
            email.attr('name', 'EMAIL');

            form.submit(function () {
                let msg;
                const successCallback = (resp) => {
                    if (resp.result === 'success') {
                        msg = 'We have sent you a confirmation email';
                        label.removeClass('error').addClass('valid');
                        email.removeClass('error').addClass('valid');
                    } else {
                        email.removeClass('valid').addClass('error');
                        label.removeClass('valid').addClass('error');
                        let index = -1;
                        try {
                            const parts = resp.msg.split(' - ', 2);
                            if (parts[1] === undefined) {
                                msg = resp.msg;
                            } else {
                                const i = parseInt(parts[0], 10);
                                if (i.toString() === parts[0]) {
                                    index = parts[0];
                                    msg = parts[1];
                                } else {
                                    index = -1;
                                    msg = resp.msg;
                                }
                            }
                        }
                        catch (e) {
                            index = -1;
                            msg = resp.msg;
                        }
                    }

                    // Translate and display message
                    if (
                        settings.language !== 'en'
                        && responses[msg] !== undefined
                        && translations
                        && translations[settings.language]
                        && translations[settings.language][responses[msg]]
                    ) {
                        msg = translations[settings.language][responses[msg]];
                    }
                    label.html(msg);

                    label.show(2000);
                    if (settings.callback) {
                        settings.callback(resp);
                    }
                };

                const data = {};
                const dataArray = form.serializeArray();
                dataArray.forEach((item) => {
                    data[item.name] = item.value;
                });

                $.ajax({
                    url,
                    data,
                    success: successCallback,
                    dataType: 'jsonp',
                    error: (resp, text) => {
                        console.log('mailchimp ajax submit error: ' + text);
                    }
                });

                // Translate and display submit message
                let submitMsg = 'Submitting...';
                if (
                    settings.language !== 'en'
                    && translations
                    && translations[settings.language]
                    && translations[settings.language]['submit']
                ) {
                    submitMsg = translations[settings.language]['submit'];
               }
                label.html(submitMsg).show(2000);

                return false;
            });
        });
    };

    $.fn.ajaxChimp = ajaxChimp;
})(jQuery);